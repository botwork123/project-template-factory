...
if ! command -v uv >/dev/null 2>&1; then
  echo "[wt_env] ERROR: uv is required but not found in PATH" >&2
  echo "[wt_env] Install uv: curl -LsSf https://astral.sh/uv/install.sh | sh" >&2
  exit 1
fi

if [[ ! -f "$CONSTRAINTS_FILE" ]]; then
  echo "[wt_env] Missing constraints file: $CONSTRAINTS_FILE" >&2
  exit 1
fi

echo "[wt_env] Using uv"
...
if [[ -z "${CI:-}" ]]; then
  ...
  if [[ -n "$HOOKS_PATH" ]]; then
    echo "[wt_env] Skipping pre-commit install (core.hooksPath=$HOOKS_PATH)"
  else
    echo "[wt_env] Installing pre-commit hooks..."
